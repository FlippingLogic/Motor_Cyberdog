...
CustomInterface::CustomInterface( const double& loop_rate )
    : _motor_data_Lcm( getLcmUrl_port( 7667, 255 ) ), _motor_ctrl_state_Lcm( getLcmUrl_port( 7667, 255 ) ), _robot_state_Lcm( getLcmUrl_port( 7669, 255 ) ),
    ...
    _user_code_SpinThread       = std::thread( &CustomInterface::Spin, this );
}
...
void CustomInterface::handle_motor_ctrl_state_LCM( const lcm::ReceiveBuffer* rbuf, const std::string& chan, const motor_ctrl_state_lcmt* msg ) {
    ( void )rbuf;
    ( void )chan;

    //std::cout<<"Motor state reading Thread Entered \n"<<std::endl;

    robot_data.err_flag            = msg->err_flag;
    robot_data.ctrl_topic_interval = msg->ctrl_topic_interval;
    mode_state                     = true;
}
void CustomInterface::handle_motor_data_LCM( const lcm::ReceiveBuffer* rbuf, const std::string& chan, const leg_control_data_lcmt* msg ) {
    ( void )rbuf;
    ( void )chan;

    //std::cout<<"Motor data reading Thread Entered \n"<<std::endl;
...
void CustomInterface::handle_robot_state_LCM( const lcm::ReceiveBuffer* rbuf, const std::string& chan, const state_estimator_lcmt* msg ) {
    ( void )rbuf;
    ( void )chan;

    //std::cout<<"Robot state reading Thread Entered \n"<<std::endl;
